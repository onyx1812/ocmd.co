...
class MyWorks_WC_QBO_Sync_Admin {
	...
	public function sync_uiiqb_cb_field($post){
		global $MSQS_QL;
		if($post->post_type != 'product' || $MSQS_QL->is_plg_lc_p_l()){return '';}
	...
	public function mw_wc_qbo_cron_schedules($schedules){
		...
		if($cit!='MWQBO_10min' && $cit!='MWQBO_30min' && $cit!='MWQBO_60min'){
			$cit = 'MWQBO_10min';
		}
		$interval = 10*60;
		$display = 10;
		if($cit=='MWQBO_30min'){$interval = 30*60;$display=30;}
		if($cit=='MWQBO_60min'){$interval = 60*60;$display=60;}
		
		if(!isset($schedules[$cit])){//30min
			$schedules[$cit] = array(
				'interval' => $interval,
				'display' => __('Once every '.$display.' minutes'));
		}
	...
	public function mwqbosync_inventory_import_schedule_cron(){
		...
		if(!wp_next_scheduled('mwqbosync_inventory_import_schedule_hook')){
			//
			if($MSQS_QL->option_checked('mw_wc_qbo_sync_ignore_cdc_for_invnt_import')){
				wp_schedule_event(time(), 'IIFMQS_30min', 'mwqbosync_inventory_import_schedule_hook');
			}else{
				wp_schedule_event(time(), 'IIFMQS_5min', 'mwqbosync_inventory_import_schedule_hook');
			}			
		}
		add_action('mwqbosync_inventory_import_schedule_hook', array($this,'mwqbosync_inventory_import_schedule_function'));		
	}
	...
	public function mw_wc_qbo_init(){
		...
		*/
		...
		if(!$MSQS_QL->is_plugin_active('inventory-import-for-myworks-qbo-sync') && !$MSQS_QL->is_plg_lc_p_l()){
	...
	public  function mw_add_meta_boxes(){
		...
		add_meta_box( 'qbo_invoice_info', __('QuickBooks Invoice Information','mw_wc_qbo_sync'), array($this,'qbo_invoice_info_fn'), 'shop_order', 'side', 'core' );
	}
	...
	public function qbo_invoice_info_fn(){
		...
		if($order_id){
			$wco_map_data_arr = $MSQS_QL->get_push_invoice_map_data(array("'{$order_id}'","'S-{$order_id}'","'OS-{$order_id}'"),$is_qbosa_sr);
		}
		...
		if(is_array($wco_map_data_arr) && count($wco_map_data_arr)){
			...
			Status: &nbsp;&nbsp;
			<a target="_blank" href="'.$qbo_href.'"><span title="'.$ie_title.'" class="mw_qbo_sync_status_span mw_qbo_sync_status_paid">Synced</span></a>
			</p>
			...
			if($MSQS_QL->is_plugin_active('split-order-custom-po-for-myworks-qbo-sync')){
				if(isset($wco_map_data_arr[1]) && is_array($wco_map_data_arr[1]) && count($wco_map_data_arr[1])){
					echo '
					<p class="mw_qbo_sync_number_p">
						Number: <span class="mw_qbo_sync_status_span mw_qbo_sync_status_info">'.$wco_map_data_arr[1]['DocNumber'].'</span>
					</p>';
				}
			}				
		}else{
...
EOF;
	...
	public static function is_trial_version_check(){
		...
		if($MSQS_QL->option_checked('mw_wc_qbo_sync_trial_license')){
			...
			<div class="text-btn mwqb_tlmb" style="position: relative;">
				...
				<a target="_blank" href="https://myworks.software/account/clientarea.php?action=productdetails&id='.$MSQS_QL->get_option('mw_wc_qbo_sync_trial_license_serviceid').'" class="btn btn-info" role="button">UPGRADE NOW!
				</a></br>
				&nbsp;
			...
			<div class="text-btn mwqb_lpmb" style="position: relative;">
				...
				<a target="_blank" href="https://myworks.software/account/clientarea.php?action=productdetails&id='.$MSQS_QL->get_option('mw_wc_qbo_sync_service_id').'" class="btn btn-info" role="button">UPGRADE NOW!
				</a></br>
				&nbsp;
				<small>Unlimited sync, Inventory support, 2-Way sync, Intelligent bank deposits and more!</small>
			</div>
			';
		}
	}
	...
	public function check_setup_status(){
		...
		if($is_setup_ok){
			...
			*/
			...
			if(!$MSQS_QL->option_checked('mw_wc_qbo_sync_pause_up_qbo_conection') && !$mw_wc_qbo_sync_default_shipping_product && !$MSQS_QL->get_qbo_company_setting('is_shipping_allowed')){
				$is_setup_ok = false;
	...
	public function mw_qbo_wc_variation_save($post_id='', $post=''){
		...
		if($variation_id && $product_id){
			...
			$variation_data['name'] = $MSQS_QL->get_variation_name_from_id($variation->post_title,'',$variation->ID);
			...
			if(!empty($variation->post_excerpt)){
				$variation_data['short_description'] = $variation->post_excerpt;
			}else{
	...
	public function myworks_wc_qbo_sync_pu_product_stock_update($post_ID, $post_after, $post_before){		
		...
		if($post_ID>0 && is_object($post_after) && !empty($post_after)){
			if(isset($post_after->post_type) && $post_after->post_type == 'product'){
				...
				if(is_object($post_before) && !empty($post_before) && $mwqb_sync_uiiqb == 1 && !$MSQS_QL->is_plg_lc_p_l()){
											...
											if(in_array($order_status,$a_os_l)){
												...
												if(!$MSQS_QL->option_checked('mw_wc_qbo_sync_pause_up_qbo_conection')){
													...
													if(!$MSQS_QL->lp_chk_osl_allwd()){
														$MSQS_QL->save_log('Export Order Error','Order Sync Limit Reached.','Invoice',0);
														return false;
													}
	...
	public function myworks_wc_qbo_sync_order_realtime($order_sync_info=''){
		...
		*/
		...
		if($or_queue_added && !$MSQS_QL->is_order_sync_as_sales_receipt($o_id)){
		...
		*/
		...
		if($order_id){
			...
			*/
			...
			if(empty($invoice_data['qbo_inv_items']) && !$is_order_has_fees){
				if(!$is_os_err){
				...
				if(($catch_all_q && $caq_not_exists) || !$MSQS_QL->check_quickbooks_invoice_get_obj($invoice_data['wc_inv_id'],$invoice_data['wc_inv_num'])){
					...
					if($ds_ori_wc_inv_id){
						...
						if($mw_qbo_inv_id>0){
							...
							/**/
							if(!$manual && !$MSQS_QL->is_order_sync_as_sales_receipt($order_id)){
								$_transaction_id = get_post_meta($order_id,'_transaction_id',true);
								$a_bti = true;
								if(!empty($_transaction_id) || $a_bti){
