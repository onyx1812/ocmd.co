...
class MyWorks_WC_QBO_Sync_QBO_Lib {
	...
	protected $quickbooks_connection_dashboard_url='https://myworks.software/account';	
				...
				if(is_array($supplier_data) && count($supplier_data)){
					...
					*/
					...
					if ($resp = $vendorService->update($Context, $realm, $vendor->getId(), $vendor)){
						...
						$log_status = 1;
						$this->save_log($log_title,$log_details,'Vendor',$log_status,true);
						...
						$log_details.="Error:$res_err";
						$this->save_log($log_title,$log_details,'Vendor',$log_status,true,true);
		...
		if($setting!='' && $this->is_connected()){
			...
			*/
			...
			switch ($setting) {
				...
				case 'is_m_currency':
					if(!$this->is_plg_lc_p_l() && $prf->countCurrencyPrefs()){
		...
		if(is_array($entities) && count($entities)){
			...
			foreach($entities as $Entity){
				...
				if(is_array($mw_wc_qbo_sync_webhook_items) && count($mw_wc_qbo_sync_webhook_items)){
					...
					//Payment Add
					if($name == 'Payment' && in_array('Payment',$mw_wc_qbo_sync_webhook_items) && !$this->is_plg_lc_p_l()){
						if(($operation == 'Create')){
	...
	public function get_mapped_qbo_items_from_wc_items($wc_items=array(),$real_time_data=false,$acof_txt='',$tax_details=array()){
		...
		if(is_array($wc_items) && count($wc_items)){
			...
			/**/
			$mw_wc_qbo_sync_inv_sr_qb_lid_val = $this->get_option('mw_wc_qbo_sync_inv_sr_qb_lid_val');
			
			if($mw_wc_qbo_sync_inv_sr_qb_lid_val == 'woo_pv_sdc'){
	...
	public function get_wc_order_details_from_order($order_id,$order){
		...
		if($order_id && is_object($order) && !empty($order)){
			...
			if($this->option_checked('mw_wc_qbo_sync_qbo_push_invoice_date')){
				$invoice_data['wc_inv_date'] = $this->now('Y-m-d');
				$invoice_data['wc_inv_due_date'] = $this->now('Y-m-d');
	...
	public function debug(){
		...
		//$this->_p($this->get_qbo_customer_email_by_id_from_db(4));
	...
	public function if_refund_exists($refund_data){
		//if($refund_data['refund_id'] == '23155' || $refund_data['wc_inv_id'] == '23154'){return false;}
		if($this->is_connected()){
	...
	public function get_qs_catch_all_order_ids(){
		...
		if($this->option_checked('mw_wc_qbo_sync_pause_up_qbo_conection')){
			...
			$qc_it_m = 0;
			if($qc_it == 'MWQBO_10min'){$qc_it_m = 10;}if($qc_it == 'MWQBO_30min'){$qc_it_m = 30;}if($qc_it == 'MWQBO_60min'){$qc_it_m = 60;}
			if(!$qc_it_m){
				$qc_it_m = 10;
			}
			$qc_it_m += 30;
	...
	public function Cron_Deposit_Sr($gateways=array(),$single_ord_id=0,$currency=array()){
		if($this->is_connected()){			
			if(!$this->is_order_sync_as_sales_receipt($single_ord_id)){
				return false;
			}
...
EOF;
		...
		if(is_array($opt_arr) && count($opt_arr)>0){
			foreach ($opt_arr as $key => $value) {
				...
				if($s_key!='' && $s_val!=''){
					...
					if($return){
						$options.='<option value="'.$value[$s_key].'" '.$sel_text.'>'.stripslashes($value[$s_val]).'</option>';
					}else{
						echo '<option value="'.$value[$s_key].'" '.$sel_text.'>'.stripslashes($value[$s_val]).'</option>';
					}
					...
					if($return){
						$options.='<option value="'.$key.'" '.$sel_text.'>'.stripslashes($value).'</option>';
					}else{
						echo '<option value="'.$key.'" '.$sel_text.'>'.stripslashes($value).'</option>';
					}
	...
	public function count_woocommerce_product_list($search_txt='',$is_inventory=false,$p_type='',$product_um_srch='') {
		...
		//
		if($this->option_checked('mw_wc_qbo_sync_hide_vpp_fmp_pages')){
	...
	/*Manage Invoice*/
	public function count_qbo_invoice_list($search_txt='',$date_from='',$date_to='',$qb_customer_id){
	...
	public function get_qbo_invoice_list($search_txt='',$limit='',$date_from='',$date_to='',$qb_customer_id){
		if($this->is_connected() && !$this->is_plg_lc_p_l()){
			$qb_customer_id = (int) $qb_customer_id;
	...
	public function count_qbo_salesreceipt_list($search_txt='',$date_from='',$date_to='',$qb_customer_id){
		if($this->is_connected() && !$this->is_plg_lc_p_l()){
			$qb_customer_id = (int) $qb_customer_id;
			if($qb_customer_id < 1){return 0;}
	...
	public function get_qbo_salesreceipt_list($search_txt='',$limit='',$date_from='',$date_to='',$qb_customer_id){
		if($this->is_connected() && !$this->is_plg_lc_p_l()){
			$qb_customer_id = (int) $qb_customer_id;
			if($qb_customer_id < 1){return array();}
	...
	public function Fix_All_WooCommerce_Variations_Names(){
		...
		if($total_v_name_changed>0){			
			$this->save_log('Variations Name Update','Number of variations name updated: '.$total_v_name_changed,'Variation_NU',1,true);
		}
	...
	public function is_queue_add($action='',$data){
