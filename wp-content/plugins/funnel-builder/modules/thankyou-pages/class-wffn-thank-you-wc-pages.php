...
class WFFN_Thank_You_WC_Pages extends WFFN_Module_Common {
    ...
    public function __construct() {
        ...
        add_action( 'wp_ajax_wffn_tp_global_settings_update', array( $this, 'update_global_settings' ) );
    ...
    public function parse_request_for_thankyou() {
        ...
        if ( is_singular( $post->post_type ) && ( self::get_post_type_slug() === $post->post_type ) ) {
    ...
    public function maybe_set_query_var( $wp_query_obj ) {
        ...
        if ( true === $this->is_wfty_page() ) {
            ...
            if ( $get_order_id === null ) {
                remove_action( 'wp', array( $this, 'validate_order' ), 11 );
                $this->data->set_fallback_order();
                $get_order    = $this->data->get_order();
                $get_order_id = ( $get_order instanceof WC_Order ) ? $get_order->get_id() : 0;
            }
    ...
    public function validate_order() {
        ...
        if ( $order instanceof WC_Order ) {
            ...
            if ( filter_input( INPUT_GET, 'key', FILTER_SANITIZE_STRING ) !== $order_key ) {
    ...
    public function duplicate_thank_you_page( $ty_page_id ) {
        ...
        if ( $ty_page_id > 0 ) {
            ...
            if ( ! is_null( $ty_page ) && ( $ty_page->post_type === $this->get_post_type_slug() || in_array( $ty_page->post_type, $this->get_inherit_supported_post_type(), true ) ) ) {
                $args         = [
                    'post_title'   => $ty_page->post_title . ' - ' . __( 'Copy', 'funnel-builder' ),
                    'post_content' => $ty_page->post_content,
                    'post_name'    => sanitize_title( $ty_page->post_title . ' - ' . __( 'Copy', 'funnel-builder' ) ),
                    'post_type'    => $this->get_post_type_slug(),
                ];
    ...
    public function load_assets() {
        ...
        if ( 'wf-ty' === $page_now ) {
            ...
            wp_enqueue_script( 'wffn_tp_js', $this->url . 'assets/js/admin.js', [], time(), true );
            ...
            if ( 'design' === filter_input( INPUT_GET, 'section', FILTER_SANITIZE_STRING ) ) {
    ...
    public function localize_data() {
        ...
        $data['nonce_global_settings'] = wp_create_nonce( 'wffn_tp_global_settings_update' );
        ...
        $data['global_setting_fields'] = array(
            'legends_texts' => array(
                ...
                'tracking_n_analytics' => __( 'Tracking & Analytics', 'funnel-builder' ),
            ...
            'fields'        => array(
                ...
                'script'                        => array(
                    ...
                    'hint'  => __( 'These scripts will be globally embedded in all the thank you Pages.', 'funnel-builder' )
                ),
                'custom_html_tracking_general_' => array(
                    'hint'         => sprintf( __( 'You can configure purchase events from <a href="%s">WooFunnels > Settings > General  </a>' ), BWF_Admin_General_Settings::get_instance()->get_settings_link() ),
                    'styleClasses' => [ 'bwf_tracking_general_label' ],
                ),
            ),
        );
            ...
            [
                ...
                "fields"       => [
                    ...
                    [
                        ...
                        'values'        => ( WFFN_Common::get_fonts_list() ),
    ...
    public function get_optionsShortCode( $key = 'all', $id = 0 ) {
        ...
        $id = empty( $id ) ? filter_input( INPUT_GET, 'preview_id', FILTER_SANITIZE_STRING ) : $id;
    ...
    public function save_design() {
        ...
        if ( $wftp_id > 0 ) {
            ...
            $this->update_page_design( $wftp_id, [
                'selected'        => $selected,
    ...
    public function update_global_settings() {
        check_admin_referer( 'wffn_tp_global_settings_update', '_nonce' );
        ...
        if ( is_array( $options ) ) {
            ...
            $options['css']    = htmlentities( $options['css'] );
        }
    ...
    public function update_custom_settings() {
        ...
        if ( isset( $options['custom_redirect_page'] ) && ! empty( $options['custom_redirect_page'] ) ) {
            update_option( 'wfty_custom_' . $options['custom_redirect_page']['id'], $thankyou_id );
        }
    ...
    public function maybe_check_for_custom_page() {
        ...
        $get_thankyou_page_id_for_this_post = get_option( 'wfty_custom_' . $post->ID, false );

        if ( empty( $get_thankyou_page_id_for_this_post ) ) {
            return;
        }
        global $wp_query;
        $this->thankyoupage_id  = $get_thankyou_page_id_for_this_post;
        $this->wfty_is_thankyou = true;
