 ...
 * @author woofunnels.
 */
class WFFN_Ecomm_Tracking_Optin {
	private static $ins = null;
	private $admin_general_settings;

	public function __construct() {
		add_action( 'wp_head', array( $this, 'render' ), 999 );
	...
	public function render_fb_view() {
		if ( $this->do_track_fb_view() && WFOPP_Core()->optin_pages->is_wfop_page() ) {
			?>
			fbq('track', 'PageView');
			<?php
		}
	...
	public function ga_code() {
		$get_ga_key = apply_filters( 'bwf_get_ga_op_key', $this->admin_general_settings->get_option( 'ga_key' ) );
