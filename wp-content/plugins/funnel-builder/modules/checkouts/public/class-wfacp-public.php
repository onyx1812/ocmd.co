...
class WFACP_Public {
	...
	protected function __construct() {
		...
		add_filter( 'woocommerce_checkout_no_payment_needed_redirect', [
			$this,
			'reset_session_when_order_processed'
		] );
	...
	public function default_value_via_url() {
		...
		if ( isset( $_GET[ $default_value_parameter ] ) && '' != $_GET[ $default_value_parameter ] ) {
			$best_value = filter_input( INPUT_GET, $default_value_parameter, FILTER_SANITIZE_STRING );
	...
	public function best_value_via_url() {
		...
		if ( isset( $_GET[ $best_value_parameter ] ) && '' != $_GET[ $best_value_parameter ] ) {
			$best_value = filter_input( INPUT_GET, $best_value_parameter, FILTER_SANITIZE_STRING );
	...
	public function woocommerce_ajax_get_endpoint( $url, $request ) {
		if ( WFACP_Common::get_id() > 0 ) {
			...
			if ( isset( $_REQUEST['currency'] ) ) {
				$query['currency'] = filter_input( INPUT_GET, 'currency', FILTER_SANITIZE_STRING );;
			}
			if ( isset( $_REQUEST['lang'] ) ) {
				$query['lang'] = filter_input( INPUT_GET, 'lang', FILTER_SANITIZE_STRING );
			}
	...
	public function set_session_when_coupon_applied() {
		...
		if ( isset( $_REQUEST['wfacp_id'] ) ) {
			$id       = filter_input( INPUT_GET, 'wfacp_id', FILTER_SANITIZE_STRING );
			$id       = absint( $id );
			$c[ $id ] = true;
		}
	...
	public function woocommerce_checkout_process() {
		...
		if ( isset( $_POST['wfacp_cart_hash'] ) && '' !== $_POST['wfacp_cart_hash'] ) {
			$wfacp_cart_hash = filter_input( INPUT_POST, 'wfacp_cart_hash', FILTER_SANITIZE_STRING );
	...
	public function add_to_cart_via_url() {
		...
		if ( isset( $_GET[ $add_checkout_parameter ] ) && '' != $_GET[ $add_checkout_parameter ] ) {
			...
			$t_add_checkout_parameter  = filter_input( INPUT_GET, $add_checkout_parameter, FILTER_SANITIZE_STRING );
			...
			if ( isset( $_GET[ $quantity_parameter ] ) ) {
				$t_quantity_parameter = filter_input( INPUT_GET, $quantity_parameter, FILTER_SANITIZE_STRING );
	...
	public function merge_default_product( $default_products, $products, $settings ) {
		...
		if ( isset( $_GET[ $default ] ) && '' !== $_GET[ $default ] ) {
			...
			if ( '' !== $data ) {
				$t_default    = filter_input( INPUT_GET, $default, FILTER_SANITIZE_STRING );
