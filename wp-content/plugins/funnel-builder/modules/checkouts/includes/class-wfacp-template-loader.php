...
final class WFACP_Template_loader {
	...
	public function is_wfacp_checkout_page() {
		...
		if ( WFACP_Common::is_checkout_process() ) {
			$wfacp_post_id = filter_input( INPUT_POST, '_wfacp_post_id', FILTER_SANITIZE_STRING );
	...
	public function load_template( $wfacp_id ) {
		...
		if ( 'elementor' === $selected_type ) {
			$template_file = WFACP_BUILDER_DIR . '/elementor/template/template.php';
		} elseif ( 'embed_forms' === $selected_type ) {
		...
		foreach ( $get_all_opted_page_builders as $builder ) {
			...
			if ( 'divi' === $builder ) {
				...
				if ( 'activated' !== $theme_status && 'activate' === $plugin_status ) {
					$plugin_string = sprintf( __( 'Please click on the button below to activate %s first and then import the template.', 'funnel-builder' ), esc_html( $page_builder['title'] ) );
					$button_text   = __( 'Activate the plugin', 'funnel-builder' );
					$no_install    = 'no';
				...
				$plugin_string .= 'Please click on the below button to activate ' . esc_html( $page_builder['title'] );
			}
