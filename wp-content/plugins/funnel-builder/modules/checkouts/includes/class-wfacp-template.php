...
abstract class WFACP_Template_Common {
	...
	private function woocommerce_field_hooks() {
		...
		add_action( 'woocommerce_checkout_order_processed', [ $this, 'woocommerce_checkout_update_order_meta' ], 10, 2 );
	}
		...
		if ( isset( $_POST['_wfacp_post_id'] ) && ! wc_string_to_bool( $this->have_billing_address ) && wc_string_to_bool( $this->have_shipping_address ) ) {
			...
			foreach ( $available_fields as $key ) {
				...
				if ( isset( $template_fields['shipping'][ $s_key ] ) ) {
					...
					$s_key_data         = filter_input( INPUT_POST, $s_key, FILTER_SANITIZE_STRING );
	...
	public function woocommerce_checkout_update_order_meta( $order_id, $posted_data ) {
		$wfacp_id       = filter_input( INPUT_POST, '_wfacp_post_id', FILTER_SANITIZE_STRING );
		...
		// Means Billing address present and no billing_same_as_shipping checkbox options available
		if ( isset( $_POST['wfacp_billing_address_present'] ) && ! isset( $_POST['billing_same_as_shipping'] ) ) {
			// $index =''  because of not copy shipping details to billing  in case of billing address present Directly
			$index = '';
		}
		...
		if ( ! isset( $_REQUEST['ship_to_different_address'] ) && apply_filters( 'wfacp_unset_shipping_first_last_name_when_ship_to_different_address_on', true, $arr_data ) ) {
			$field = $this->get_checkout_fields();
			// if shipping first anf last name already present then unset  shipping_first_name and shipping_last_name
			if ( isset( $field['shipping']['shipping_first_name'] ) ) {
				unset( $arr['shipping_first_name'] );
			}
			if ( isset( $field['shipping']['shipping_last_name'] ) ) {
				unset( $arr['shipping_last_name'] );
			}
		}

		foreach ( $arr as $a_key => $second_key ) {
			if ( isset( $_REQUEST[ $a_key ] ) && '' !== $_REQUEST[ $a_key ] ) {
				$arr_data[ '_' . $a_key ] = filter_input( INPUT_POST, $a_key, FILTER_SANITIZE_STRING );
				continue;
			}
			if ( isset( $_REQUEST[ $second_key ] ) && '' !== $_REQUEST[ $second_key ] ) {
				$arr_data[ '_' . $a_key ] = filter_input( INPUT_POST, $second_key, FILTER_SANITIZE_STRING );
				continue;
			}
		}
		...
		if ( isset( $_REQUEST['wfacp_source'] ) ) {
			$wfacp_source = filter_input( INPUT_POST, 'wfacp_source', FILTER_SANITIZE_STRING );
		...
		if ( $wfacp_id > 0 ) {
			...
			$this->update_custom_fields( $order_id, $posted_data );
		}
	...
	public function assign_first_last_name() {
		...
		foreach ( $arr as $a_key => $second_key ) {
			...
			if ( isset( $_REQUEST[ $second_key ] ) && '' !== $_REQUEST[ $second_key ] ) {
				$_REQUEST[ $a_key ] = filter_input( INPUT_POST, $second_key, FILTER_SANITIZE_STRING );
				continue;
			}
		}
	...
	public function pre_populate_from_get_parameter( $value, $key, $field ) {
		...
		if ( isset( $_REQUEST[ $key ] ) ) {
			$key_data  = filter_input( INPUT_GET, $key, FILTER_SANITIZE_STRING );
