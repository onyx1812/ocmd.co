...
final class WFACP_admin {
	...
	public function set_section() {
		if ( WFACP_Common::get_id() > 0 && isset( $_GET['section'] ) ) {
			...
			$this->current_page = filter_input( INPUT_GET, 'section', FILTER_SANITIZE_STRING );
	...
	public function register_admin_menu() {
		...
		add_submenu_page( 'woofunnels', 'Checkouts', 'Checkouts', 'manage_options', 'wfacp', [
			$this,
			'admin_page',
		] );
	}
	...
	public function admin_enqueue_assets() {
		...
		if ( isset( $_REQUEST['page'] ) && $_REQUEST['page'] == 'wfacp' ) {
			...
			if ( filter_input( INPUT_GET, 'section', FILTER_SANITIZE_STRING ) === 'bwf_settings' ) {
	...
	public function design_settings() {
		...
		$google_fonts     = json_decode( file_get_contents( WFACP_WEB_FONT_PATH . "/fonts.json" ) );
		$web_fonts        = apply_filters( 'wfacp_customizer_fonts', $google_fonts );
		$typography_fonts = [];
		foreach ( $web_fonts as $font_key => $font_name ) {
			$typography_fonts[] = [ 'id' => $font_name, 'name' => $font_name ];
		}
...
#order_data #wfacp_admin_advanced_field span.wfacp_radio_options_group:after{    clear: both;} </style>";
	...
	public function add_meta_boxes_for_make_page_as_checkout() {
		add_meta_box( 'woofunnels-aero-checkout', __( 'Aero Checkout', 'woofunnels-aero-checkout' ), [
			$this,
			'render_my_meta_box'
		], 'page', 'side', 'default' );
	}
	...
	public function add_meta_boxes_for_shortcodes() {
		$post_type = WFACP_Common::get_post_type_slug();
		add_meta_box( 'woofunnels-aero-checkout-shortcode', __( 'Aero Checkout', 'woofunnels-aero-checkout' ), [
			$this,
		...
		if ( $id > 0 ) {
			...
			<div class="wfacp_shortcode">
				<div class="wfacp_shortcode_inner">
					<div class="wfacp_description">
						<label for='wfacp_shortcode_normal'><?php esc_html_e( 'Form Shortcode', 'woofunnels-aero-checkout' ); ?></label>
						<input type="text" readonly="readonly" id='wfacp_shorcode_normal' style="width: 100%;" value="<?php echo $normal ?>">
					</div>
					<a href="javascript:void(0)" class="wfacp_copy_text"><?php esc_html_e( 'Copy', 'woofunnels-aero-checkout' ); ?></a>
				</div>
			...
			<script id="wfacp-back-button-template" type="text/html">
				<div id="wfacp-switch-mode">
					<a id="elementor-back-button" class="button button-default button-large"
					   href="<?php echo esc_url( $edit_link ); ?>">
						<?php echo __( '&#8592; Back to Checkout', 'woofunnels-aero-checkout' ); ?>
					</a>
				</div>
			</script>
	...
	public function add_back_button() {
		...
		<div id="wfacp-switch-mode">
			<a id="wfacp-back-button" class="button button-default button-large"
			   href="<?php echo esc_url( $edit_link ); ?>">
