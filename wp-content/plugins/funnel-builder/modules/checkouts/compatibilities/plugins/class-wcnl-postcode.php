...
class WFACP_Compatibility_With_Wcnl_Postcode {
	...
	public function toev_field( $posted_data ) {
		...
		if ( isset( $_REQUEST['billing_house_number_suffix'] ) ) {
			...
			$posted_data['billing_house_number_suffix'] = filter_input( INPUT_POST, 'billing_house_number_suffix', FILTER_SANITIZE_STRING );
		}
		if ( isset( $_REQUEST['shipping_house_number_suffix'] ) ) {
			$posted_data['shipping_house_number_suffix'] = filter_input( INPUT_POST, 'shipping_house_number_suffix', FILTER_SANITIZE_STRING );
		}
