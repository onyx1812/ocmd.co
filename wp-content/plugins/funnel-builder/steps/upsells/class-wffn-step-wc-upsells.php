...
class WFFN_Step_WC_Upsells extends WFFN_Step {
	...
	public function __construct() {
		...
		add_filter( 'wfocu_add_control_meta_query', [ $this, 'exlude_from_query' ] );
		...
		if ( $upsell_step_id > 0 && $funnel_id > 0 ) {
			...
			if ( 'cartflows_step' === $post_type ) {
				...
				if ( $duplicate_upsell_id > 0 ) {
					...
					if ( ! empty( $new_title ) ) {
						$arr = [ 'ID' => $duplicate_upsell_id, 'post_title' => $new_title ];
						wp_update_post( $arr );
					}
...
if ( class_exists( 'WFFN_Core' ) && class_exists( 'WFOCU_Core' ) && defined( 'WOOCOMMERCE_VERSION' ) ) {

	if ( ! version_compare( WFOCU_VERSION, apply_filters( 'wffn_upsells_min_version_required', '2.2' ), '>=' ) ) {
		wffn_show_notice( array( 'pname' => WFOCU_FULL_NAME ), 'version_mismatch' );
