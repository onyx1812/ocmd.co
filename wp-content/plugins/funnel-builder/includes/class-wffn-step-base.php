...
abstract class WFFN_Step_Base {
    ...
    public function delete_step( $funnel_id, $step_id ) {
        ...
        return $funnel->delete_step( $funnel_id, $step_id );
    }
    ...
    public function copy_metadata( $id, $metadata, $excluded = [] ) {
        ...
        if ( ! empty( $metadata ) ) {
            ...
            foreach ( $metadata as $key => $meta_val ) {
                 ...
                 */
                $meta_value = addslashes( maybe_serialize( $meta_val ) );

                $sql_query_selects[] = "SELECT $id, '$meta_key', '$meta_value'"; //db call ok; no-cache ok; WPCS: unprepared SQL ok.
            }
