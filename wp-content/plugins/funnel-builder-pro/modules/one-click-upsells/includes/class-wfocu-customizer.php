...
.ast-control-tooltip {display: none !important;}</style>';
	...
	public function load_template( $offer_data ) {
		...
		if ( ! empty( $offer_data ) ) {
			...
			if ( count( get_object_vars( $offer_data ) ) > 0 ) {
				...
				if ( false === $this->is_valid_customizer_template( $this->template ) ) {
					$this->template = WFOCU_Core()->template_loader->get_default_template( $offer_data );
				}
				if ( $this->template !== '' ) {

					$locate_template = WFOCU_Core()->template_loader->get_template( $this->template );
					if ( is_array( $locate_template ) && file_exists( $locate_template['path'] ) ) {

						$this->template_ins = include_once $locate_template['path']; // phpcs:ignore WordPressVIPMinimum.Files.IncludingFile.UsingVariable
	...
	public function is_valid_customizer_template( $selected = 'sp-classic' ) {

		$customizer_group = WFOCU_Core()->template_loader->get_group( 'customizer' );
		$allTemplates     = $customizer_group->get_templates();
		 ...
		 */
		if ( ('mp-grid' === $selected && class_exists('WFOCU_MultiProductCore')) || ('mp-list' === $selected && class_exists('WFOCU_MultiProductCore')) ) {
			return true;
		}
