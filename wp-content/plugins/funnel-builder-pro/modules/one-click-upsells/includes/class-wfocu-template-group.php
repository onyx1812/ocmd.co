		...
		if ( ! empty( $offer_data ) ) {
			...
			if ( count( get_object_vars( $offer_data ) ) > 0 ) {
				...
				if ( WFOCU_Core()->template_loader->template !== '' ) {
					...
					$locate_template = WFOCU_Core()->template_loader->get_template( WFOCU_Core()->template_loader->template );
					if ( is_array( $locate_template ) && file_exists( $locate_template['path'] ) ) {

						WFOCU_Core()->template_loader->template_ins     = include_once $locate_template['path'];    // phpcs:ignore WordPressVIPMinimum.Files.IncludingFile.UsingVariable
