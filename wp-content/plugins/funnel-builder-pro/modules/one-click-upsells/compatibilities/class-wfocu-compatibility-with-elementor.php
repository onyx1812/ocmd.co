...
class WFOCU_Compatibility_With_Elementor {
	...
	public function register_tags() {
		...
		if ( $offer_id > 0 && 'elementor' !== $this->get_offer_template_group( $offer_id ) ) {
			return;
		}
	...
	public function is_elementor_offer_page() {
		$offer_id = $this->get_elementor_offer_page_id();
		if ( $offer_id > 0 ) {
			$template_group = $this->get_offer_template_group( $offer_id );
			if ( 'elementor' === $template_group && class_exists( '\Elementor\Plugin' ) ) {
				return true;
			}
		}
	...
	public function get_offer_template_group( $offer_id ) {
		$template_group = '';
		if ( $offer_id > 0 ) {
			$get_offer_meta = WFOCU_Core()->offers->get_offer_meta( $offer_id );
			$template_group = isset( $get_offer_meta->template_group ) ? $get_offer_meta->template_group : $template_group;
		}
