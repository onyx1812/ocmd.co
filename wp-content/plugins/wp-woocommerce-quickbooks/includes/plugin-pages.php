...
if($post_type == 'shop_order'){
  ...
  */
  ...
  echo '<li><a href="' . admin_url( 'admin.php?page=wc-settings&tab=' . $this->id . '&section=' . sanitize_title( $id ) ) . '" class="' . ( $current_section == $id ? 'current' : '' ) . '" title="'.sanitize_text_field($label).'">' . $label . '</a> ' . ( end( $array_keys ) == $id ? '' : '|' ) . ' </li>';
  }
...
$label='';
if(isset($v['placeholder'])){ $label=$v['placeholder']; }
else if(isset($v['label'])){ $label=$v['label']; }
...
if(isset($v['placeholder'])){ $label=$v['placeholder']; }
 ...
 $user=array('__vx_wp-user_login'=>array('label'=>'user_login'),'__vx_wp-ID'=>array('label'=>'user_id')
  ...
  $tx_arr=array();
  $prpduct_attrs=array('name'=>'Product Name','description'=>'Product Description','short_description'=>'Short Description','sku'=>'Product SKU','price'=>'Product Price','regular_price'=>'Product Regular Price','sale_price'=>'Product Sales Price','total_qbooks'=>'Product Total Sales','stock_quantity'=>'Stock Quantity','weight'=>'Product Weight','length'=>'Product Length');
  
  foreach($prpduct_attrs as $k=>$v){
  ...
  */
  ...
  ?>
  <div class="alert_danger"><?php _e('Please Select Object','wp-woocommerce-quickbooks')?></div>
  <?php
  return;
  }
...
if($object_match && $account_match && $feed_match ){    
  ...
  */
  ...
  $html="<option value=''>".__("Select Object",'wp-woocommerce-quickbooks')."</option>";
  $res=array();
...
$sql.=' order_id=%d order by id desc limit %d';
  ...
  */
  ...
  ?>
  <div class="alert_danger"><?php _e('Please Select a QuickBooks Account','wp-woocommerce-quickbooks')?></div>
  <?php
  die();
  }
