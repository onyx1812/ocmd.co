...
$json['estimate']='{ 
    ...
    "TransactionLocationType":{"label":"Transaction Location Type","options":["WithinFrance","FranceOverseas","OutsideFranceWithEU","OutsideEU"]},
...
$bool=array('ApplyTaxAfterDiscount','Taxable','Job','IsProject','Active','ProcessPayment');
$req=array('PrimaryEmailAddr_Address'); //,'CustomerRef'
$search=array('GivenName','FamilyName','FullyQualifiedName','PrimaryEmailAddr_Address','DocNumber','DisplayName'); //,'BillEmail_Address','PrivateNote'
$date=array('ExpirationDate','TxnDate','AcceptedDate','ShipDate_date');
  ...
  foreach($arr as $k=>$v){
     $type='text';
      if($k == 'PrimaryEmailAddr_Address'){
       ...
       if($k == 'PrimaryPhone_FreeFormNumber'){
        $type='phone';  
      }else
       if($k == 'WebAddr_URI'){
        $type='url';  
...
echo json_encode($search_res); die();*/
 ...
 //$res=$this->post_crm_arr('item/40');    var_dump($res); die();
  ...
  $fields_info=array();  $extra=$post=array();
...
if(empty($meta['new_entry'])){
    $sent=true;
if( isset($this->id) && $this->id == 'vxc_qbooks' && !in_array($object,array('payment','customer'))){
  ...
  if(!empty($item['tax_id'])){
      ...
      if(!empty($meta['class'])){
          $item_arr['ClassRef']=array('value'=>$meta['class']);
              ...
              if($item['is_taxable'] && !empty($item['tax_id'])){
         $tax_id=$item['tax_id'];
...
$_order=self::$_order;
  if(method_exists($_order,'get_item_total')){
       $total=$_order->get_item_total($item,false,true);
    }
$extra=$re=array();
...
foreach($item0['ItemGroupDetail']['ItemGroupLine'] as $vv){
   ...
   if(!empty($res['QueryResponse']['Item'][0]['Id'])){
    ...
    $post=array('Name'=>$name,'Sku'=>$sku,'UnitPrice'=>$unit_price,'Type'=>'NonInventory');
    $post['Taxable']=$product->is_taxable();
