...
<span style="display: table-cell; padding-left: 10px; vertical-align: middle;"><a href="#" class="notice-dismiss" title="<?php _e('Dismiss Notice','wp-woocommerce-quickbooks') ?>">dismiss</a></span> </p></div>
  <?php
     }else{
  ?>
...
if($note['status'] == '1'){
  ...
  $msg.=' - <a href="'.$log_url.'" class="vx_log_link" title="'.__('View Detail','wp-woocommerce-quickbooks').'" data-id="'.$note['log_id'].'">'.__('View Detail','wp-woocommerce-quickbooks')."</a>";
  } 
  return $msg;
  }
  /**
...
$page_url = ( is_ssl() ? 'https://' : 'http://' ) . $_SERVER['HTTP_HOST'] . $_SERVER['REQUEST_URI']; 
  ...
  $email_info=array("msg"=>$res['error'],"title"=>__("QuickBooks Error",'wp-woocommerce-quickbooks'),"info_title"=>"More Detail","info"=>array("Order Id"=>$order_url,"Time"=>date('d/M/y H:i:s',current_time('timestamp')),"Page URL"=>'<a href="'.$page_url.'" style="word-break:break-all;">'.$page_url.'</a>'));
...
if(in_array($type,array("field","custom"))){ 
  ...
  if($this->user == ""){ //get user if not set
  $user_id='';
  if(isset($order['_customer_user'][0])){
  $user_id=$order['_customer_user'][0];
