...
.crm_panel .vx_error{
  ...
  <div id="crm_fields_map" class="vx_contents">
    <div class="vx_div ">
  <div class="vx_head ">
<div class="crm_head_div"> <?php _e('1. Select QuickBooks Account',  'wp-woocommerce-quickbooks' ); ?></div>
...
<div class="crm_clear"></div> 
  ...
  <div class="vx_row">
  <div class="vx_col1">
  <label for="vxc_account"><?php _e('Select QuickBooks Account.', 'wp-woocommerce-quickbooks');  $this->tooltip($tooltips['sel_object']); ?></label>
  </div>
  <div class="vx_col2">
  <div>
  ...
  <div id="crm_ajax_account" style="display:none; text-align: center; line-height: 100px;"><i><?php _e('Loading, Please Wait...', 'wp-woocommerce-quickbooks'); ?></i></div>
    <?php 
    $valid=true;
...
//post validation
 ...
 if($(".vx_required").length){
   ...
   var value=""; 
    ...
    attach_error.append('<div class="entry_row vx_entry_error"><div class="vx_error"><i class="fa fa-warning"></i> <?php _e('This is a required field','wp-woocommerce-quickbooks') ?></div></div>');
    return false;
    }  
   })  
 }
  ...
  $.post(ajaxurl,{action:'refresh_data_<?php echo $this->id ?>',vx_crm_ajax:vx_crm_ajax,vx_action:action,account:account,post_id:post_id},function(res){
 ...
 if(re.status == "ok"){
  ...
  if(field_name_select.length){
    ...
    if(field_name == '' || crm_fields[field_name] == ''){
     alert('<?php _e('Please Select Field Name','wp-woocommerce-quickbooks') ?>');
     return;   
    }  
...
if(crm_fields[name].options && typeof crm_fields[name].options == 'object'){
...
if(type){ row.find('.vxc_field_option').hide(); }
  ...
  e.preventDefault(); 
  if(!confirm('<?php _e('Are you sure to remove ?','wp-woocommerce-quickbooks') ?>')){
      return;
  }
...
start_tooltip();
  ...
  if(object == ""){
  alert("<?php _e('Please Select Object','wp-woocommerce-quickbooks') ?>");
  return;
  }
  if(vx_ajax && vx_ajax.abort){
    vx_ajax.abort();  
...
$('#vx_add_fields_select').select2({ placeholder: '<?php _e('Select Field','wp-woocommerce-quickbooks') ?>'});
add_note_sel();
} 
  });
function add_note_sel(){
    jQuery('#crm_note_fields').select2({ placeholder: '<?php _e('Select Field','wp-woocommerce-quickbooks') ?>'});
}  
