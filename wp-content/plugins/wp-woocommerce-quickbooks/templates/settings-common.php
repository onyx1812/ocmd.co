...
display: inline; 
  ...
  jQuery(document).ready(function($){
    ...
    if(unsaved) return '<?php _e('Changes you made may not be saved','wp-woocommerce-quickbooks'); ?>';
});
          // Tooltips
  var tiptip_args = {
  'attribute' : 'data-tip',
  ...
  $(this).text('<?php _e('Show Key','wp-woocommerce-quickbooks') ?>');  
  $(this).removeClass('vx_hidden');
  key.attr('type','password');  
  }else{
  $(this).text('<?php _e('Hide Key','wp-woocommerce-quickbooks') ?>');  
