...
.tablenav .tablenav-pages a,.tablenav-pages-navspan{display:inline-block;min-width:17px;border:1px solid #ccc;padding:3px 5px 7px;background:#e5e5e5;font-size:16px;line-height:1;font-weight:400;text-align:center}
  ...
  <h2 style="padding-bottom: 12px; line-height: 36px">  <img alt="<?php _e("QuickBooks Feeds", 'wp-woocommerce-quickbooks') ?>" title="<?php _e("QuickBooks Feeds", 'wp-woocommerce-quickbooks') ?>" src="<?php echo self::$base_url?>images/<?php echo $this->crm_name ?>-crm-logo.png" style="float:left; margin:0 7px 0px 0;" width="70" /> <?php _e("QuickBooks Log", 'wp-woocommerce-quickbooks'); ?></h2>
...
<input type="text" placeholder="<?php _e('Search','wp-woocommerce-quickbooks') ?>" value="<?php echo $this->post('search'); ?>" name="search" class="crm_input_inline">
  <?php
...
?>
  ...
  <option value=""><?php _e('All Objects','wp-woocommerce-quickbooks') ?></option>
  <?php
  ...
  <option value=""><?php _e('All Status','wp-woocommerce-quickbooks') ?></option>
  <?php    
  ...
  <option value=""><?php _e('All Times','wp-woocommerce-quickbooks') ?></option>
  <?php
  ...
  <input type="text" name="start_date" placeholder="<?php _e('From Date','wp-woocommerce-quickbooks') ?>" value="<?php echo $this->post('start_date');?>" class="vxc_date crm_input_inline" style="width: 100px">
  <input type="text" class="vxc_date crm_input_inline" value="<?php echo $this->post('end_date');?>" placeholder="<?php _e('To Date','wp-woocommerce-quickbooks') ?>" name="end_date"  style="width: 100px">
  </span>
 
  <button type="submit" class="button-secondary button crm_input_inline" title="<?php _e('Search','wp-woocommerce-quickbooks') ?>"><i class="fa fa-search"></i> <?php _e('Search','wp-woocommerce-quickbooks') ?></button> 
  ...
  <button type="submit" class="button-secondary button crm_input_inline" title="<?php _e('Apply','wp-woocommerce-quickbooks') ?>" id="vx_apply_bulk"><i class="fa fa-check"></i> <?php _e('Apply','wp-woocommerce-quickbooks') ?></button>

  <?php          if(count($results)>0){
  ?>
  <button type="button" name="tab_action" id="vx_export" title="<?php _e('Export as CSV','wp-woocommerce-quickbooks') ?>" class="button-secondary button crm_input_inline vx_left_10"><i class="fa fa-download"></i> <?php _e('Export as CSV','wp-woocommerce-quickbooks') ?></button> 
  <?php 
  ...
  if(isset($results[0]['order_id']) && $results[0]['order_id']!=""){
     ...
     $parent_id=(int)$results[0]['parent_id'];
      ?>
  <a href="<?php echo admin_url( 'admin.php?page='.$this->id.'_log&order_id='.$order_id);?>" title="<?php echo sprintf(__('View Order# %s Logs','wp-woocommerce-quickbooks'),$order_id); ?>" class="button vx_left_10"><i class="fa fa-hand-o-right"></i> <?php 
  if($parent_id<0){
  echo sprintf(__('View Visitor# %s Logs','wp-woocommerce-quickbooks'),$order_id); 
  }else{
  echo sprintf(__('View Order# %s Logs','wp-woocommerce-quickbooks'),$order_id); 
  }
   ?></a><?php
  }}
 if($this->post('order_id') !="" || $this->post('id') !=""){
          ?><a href="<?php echo admin_url( 'admin.php?page='.$this->id.'_log');?>" title="<?php _e('View All Logs','wp-woocommerce-quickbooks') ?>" class="button vx_left_10"><i class="fa fa-external-link"></i> <?php _e('View All Logs','wp-woocommerce-quickbooks') ?></a>        
          <?php
      }
  ?>
  <div class="tablenav-pages">
  <span id="paging_header" class="displaying-num"><?php _e('Displaying','wp-woocommerce-quickbooks') ?> <span id="paging_range_min_header"><?php echo $range_min?></span> - <span id="paging_range_max_header"><?php echo $range_max; ?></span> of <span id="paging_total_header"><?php echo $items;?></span></span>
  ...
  <th scope="col" class="manage-column vx_sort"  data-name="crm_id"><?php _e('QuickBooks ID', 'wp-woocommerce-quickbooks') ?>
  <i class="fa fa-caret-<?php echo $crm_order ?> vx_sort_icon <?php echo $crm_class ?>"></i>                          
  </th>
  <th scope="col" class="manage-column vx_sort"  data-name="order_id"><?php _e('Order ID', 'wp-woocommerce-quickbooks') ?>
  <i class="fa fa-caret-<?php echo $entry_order ?> vx_sort_icon <?php echo $entry_class ?>"></i>                      
  </th>
  <th scope="col" class="manage-column"  data-name="object"><?php _e('Description', 'wp-woocommerce-quickbooks') ?>
  </th>
  <th scope="col" class="manage-column vx_sort"  data-name="time"><?php _e('Time', 'wp-woocommerce-quickbooks') ?>
  <i class="fa fa-caret-<?php echo $time_order ?> vx_sort_icon <?php echo $time_class ?>"></i>
  </th>
  <th style="width: 40px"><?php _e('Detail','wp-woocommerce-quickbooks') ?></th>
  </tr>
  </thead>
  ...
  <th><?php _e('Detail','wp-woocommerce-quickbooks') ?></th>  
  </tr>
  ...
  <td class="vx_check_col"><input type="checkbox" name="log_id[]" value="<?php echo $row['id'] ?>" class="crm_input_check"></td>
    <td class="vx_icon_col"><img src="<?php echo self::$base_url ?>images/<?php echo $row["status_img"] ?>.png" alt="<?php echo $row['status'] ? __('Active', 'wp-woocommerce-quickbooks') : __('Inactive', 'wp-woocommerce-quickbooks');?>" title="<?php echo $row['title'];?>" class="crm_status_img" /></td>
  <td class="column-name" style="width:40%"><?php echo $row['a_link'] ?></td>
    ...
    <td><i class="vx_icons vx_detail fa fa-th-list" title="<?php _e('Expand Detail','wp-woocommerce-quickbooks'); ?>"></i></td>  
  </tr>
  ...
  <tr>
    <td colspan="4" style="padding:20px;">
        <?php _e('No Record(s) Found', 'wp-woocommerce-quickbooks'); ?>
    </td>
  </tr>
  <?php
  }
  ...
  ?>
    <div class="crm_actions tablenav">
   <a id="vx_clear_logs" class="button" title="<?php _e('Clear QuickBooks Log','wp-woocommerce-quickbooks') ?>" href="<?php echo wp_nonce_url(admin_url('admin.php?page='.$this->post('page')."&view=log&vx_tab_action_".$this->id."=clear_logs"),'vx_nonce','vx_nonce'); ?>"><?php _e('Clear QuickBooks Log','wp-woocommerce-quickbooks') ?></a>
  <div class="tablenav-pages">   <span id="paging_header" class="displaying-num"><?php _e('Displaying','wp-woocommerce-quickbooks') ?> <span id="paging_range_min_header"><?php echo $range_min?></span> - <span id="paging_range_max_header"><?php echo $range_max; ?></span> of <span id="paging_total_header"><?php echo $items;?></span></span>
  <?php  echo $page_links;?></div>
    </div>
  <?php
  }
...
if(sel.val() == ""){
    alert('<?php _e('Please Select Action','wp-woocommerce-quickbooks') ?>');
      return false;
}
if($(".crm_input_check:checked").length == 0){ 
    alert('<?php _e('Please select at least one entry','wp-woocommerce-quickbooks') ?>');
    return false;
}
var action=sel.val();
if( $.inArray(action,["send_to_crm_bulk_force","send_to_crm_bulk"]) !=-1 && $(".crm_input_check:checked").length>4){
 if(!confirm('<?php _e('Exporting more than 4 entries may take too long. Are you sure you want to continue?','wp-woocommerce-quickbooks') ?>')){
  e.preventDefault();    
 }   
}
  })
   $("#vx_clear_logs").click(function(e){
      if(!confirm('<?php _e('QuickBooks Logs will be deleted permanently. Do you want to continue?','wp-woocommerce-quickbooks') ?>')){
          e.preventDefault();
      }  
  })    
  ...
  $(document).on("click",".vx_detail,.vx_close_detail",function(e){
    ...
    if(is_main){
    icon.attr('title','<?php _e('Expand Detail','wp-woocommerce-quickbooks'); ?>');    
    }        
     return;   
    }else{
        next_tr.show('fast'); 
    next_tr.find('.vxa_entry').slideDown('fast');
       if(is_main){
    icon.attr('title','<?php _e('Collapse Detail','wp-woocommerce-quickbooks'); ?>');    
    }    
    }
