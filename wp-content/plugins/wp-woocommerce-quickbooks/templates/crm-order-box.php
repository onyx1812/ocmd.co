...
ul.vx_notes li.customer-note .note_content:after {
  ...
  ?>
  <p>
  <button class="button" type="submit" name="<?php echo $this->id ?>_send" title="<?php _e("Send to QuickBooks",'wp-woocommerce-quickbooks')?>" value="yes"><?php _e("Send to QuickBooks",'wp-woocommerce-quickbooks')?></button>
  <?php
      if($comments ){
  $log_url=admin_url( 'admin.php?page='.$this->id.'_log&order_id='.$post_id); 
  ?>
  <a href="<?php echo $log_url ?>" class="button" title="<?php _e("Go to Logs",'wp-woocommerce-quickbooks')?>"><?php _e("Go to Logs",'wp-woocommerce-quickbooks')?></a>
  <?php
      }
  ?>
  </p>