(function(){
  ...
  resps.forEach(resp => {
    resp.addEventListener('click', e => {
      ...
      if ( document.querySelector('.resp-tab.active')!=null){
      document.querySelector('.resp-tab.active').classList.remove('active');
      }
      if ( document.querySelector('.resp-nav li.active')!=null){
  ...
  const respsa = document.querySelectorAll('[data-respa]');
  respsa.forEach(resp => {
    resp.addEventListener('click', e => {
      e.preventDefault();
      var closing= e.target.classList.contains('active');
      const res = '.'+e.target.dataset.respa;
      if ( document.querySelector('.resp-tab.active')!=null){
      document.querySelector('.resp-tab.active').classList.remove('active');
      }
